...
def post_order_search(node):
    """
    Perform a post-order traversal on the tree and print the nodes.
    ...
    Args:
        node (str): The current node to start traversal from.

    Returns:
        None
    """
    # If the current node has no children, return immediately
    if not tree[node]:
        print(node)
        return

    # Recursive traversal for all children of the current node
    for child in tree[node]:
        post_order_search(child)
